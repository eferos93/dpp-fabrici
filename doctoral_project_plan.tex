...
Original schema matching techniques can be categorized as it follows:
\begin{itemize}
   \item \textbf{Schema-level matchers}, where only the metadata is considered (e.g. column labels, data-types).
   \item \textbf{Content-level matchers}, where the content of the columns is used for matching by using probabilistic approaches \cite*{Szymczak2016,Dasu2002}.
   \item \textbf{Hybrid matchers} that combine the two matchers just described.
\end{itemize}

In schema matching the problem of \textit{volume} does not take place, namely there is no need to increase computational performance, rather 
...
Up to 2017, a lot of Privacy-Preserving Record Linkage (PPRL) protocols proposed for secure two-party private record linkage were not able to meet the following three requirements altogether: (1) \textbf{full end-to-end privacy}, 
(2) \textbf{perfect precision and recall} for the matching records and (3) \textbf{sub-quadratic computational complexity} \cite*{He2017}. 
...
and does not take into account the multi-party scenario, where the problem of 
\textit{volume}, \textit{veracity} and \textit{velocity} needs to be addressed \cite*{Vatsalan2017}.
Although there have been many prior works that looked
into different security and privacy aspects of PPRL, there still remain major challenges that need to be addressed. 
As a starting point, more research is needed to understand membership inference and attribute inference attacks on PPRL techniques.
